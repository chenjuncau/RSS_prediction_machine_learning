...
Topnum <- 50
num <- sample(dim(sub_data)[1]-1,Topnum)+1
...
# c<-try(MLearn(Phe ~ ., data=RSS, svmI, TrainInd)@testScores,T)
# if(any(nchar(c)>40)){next}

# prediction model.  start here
knnf = MLearn( Phe ~ ., data=RSS, randomForestI, TrainInd,ntree=600)  # random Forest
# confuMat(knnf) 
Accuracy[i*5-4,1]="Random Forest"
Accuracy[i*5-4,2]=(confuMat(knnf)[1,2]+confuMat(knnf)[2,1])/(confuMat(knnf)[1,1]+confuMat(knnf)[1,2]+confuMat(knnf)[2,1]+confuMat(knnf)[2,2])
# knnf <- MLearn(Phe ~ ., data=RSS, svmI, TrainInd)    # SVM
# confuMat(knnf) 
# Accuracy[i*5-3,1]="SVM"
# Accuracy[i*5-3,2]=(confuMat(knnf)[1,2]+confuMat(knnf)[2,1])/(confuMat(knnf)[1,1]+confuMat(knnf)[1,2]+confuMat(knnf)[2,1]+confuMat(knnf)[2,2])
knnf = MLearn( Phe ~ ., data=RSS, naiveBayesI,TrainInd)  # naiveBayes
# confuMat(knnf)  # on held-out test data
Accuracy[i*5-2,1]="Naive Bayes"
Accuracy[i*5-2,2]=(confuMat(knnf)[1,2]+confuMat(knnf)[2,1])/(confuMat(knnf)[1,1]+confuMat(knnf)[1,2]+confuMat(knnf)[2,1]+confuMat(knnf)[2,2])
knnf = MLearn( Phe ~ ., data=RSS, knnI(k=1,l=0),TrainInd)  # knnI
# confuMat(knnf)
Accuracy[i*5-1,1]="KNN"
Accuracy[i*5-1,2]=(confuMat(knnf)[1,2]+confuMat(knnf)[2,1])/(confuMat(knnf)[1,1]+confuMat(knnf)[1,2]+confuMat(knnf)[2,1]+confuMat(knnf)[2,2])
knnf = MLearn( Phe ~ ., data=RSS, rpartI, TrainInd) # rpart
# confuMat(knnf)  # on held-out test data
Accuracy[i*5,1]="RPRT"
Accuracy[i*5,2]=(confuMat(knnf)[1,2]+confuMat(knnf)[2,1])/(confuMat(knnf)[1,1]+confuMat(knnf)[1,2]+confuMat(knnf)[2,1]+confuMat(knnf)[2,2])
print(i)
}
...
colnames(Accuracy)=c("Method","Acc")
# Accuracy2=subset(Accuracy, !is.na(Method) | !is.na(Acc))

options(drop.unused.levels=TRUE)
Accuracy2=subset(Accuracy,Method != "NA",drop = TRUE)
dim(Accuracy2)/5


Accuracy2 <- droplevels(Accuracy2)
Accuracy2$Method=factor(Accuracy2$Method)
Accuracy2$Acc=as.numeric(unlist(as.character(Accuracy2$Acc)))*100

# Accuracy3 <- array(0,c(dim(Accuracy2)[1],2))
# for(i in 1:dim(Accuracy2)[1])
# {
# Accuracy3[i,1] <- unlist(as.character(Accuracy2[i,1]))
# Accuracy3[i,2] <- as.numeric(unlist(as.character(Accuracy2[i,2])))*100
# }
# Accuracy3=as.data.frame(Accuracy3)
# colnames(Accuracy3)=c("Method","Acc")
# Accuracy3$Method=factor(Accuracy3$Method)
# Accuracy3$Acc=as.numeric(unlist(as.character(Accuracy3$Acc)))
# plot 1


pdf_name <- "Boxplot_for_RSS_5_prediction_methods_70_randome_SNP_sets.pdf"
pdf(pdf_name,width=7,height=7)
boxplot(Acc~Method,data=Accuracy2, main="Prediction by 50 simulations",
   xlab="Method", ylab="Error rate (%)",na.action = NULL)
